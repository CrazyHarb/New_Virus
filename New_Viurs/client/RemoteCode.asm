...
_szGetModuleHandle  db  'GetModuleHandleA',0
@@:
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
_RemoteThread	proc	uses ebx edi esi
		local	@hModule
...
;********************************************************************
		...
		_invoke	[ebx + _lpLoadLibrary],eax
		mov	@hModule,eax
		...
		.endw
;********************************************************************
        lea eax,[ebx + _szKernel32]
        _invoke	[ebx + _lpLoadLibrary],eax
		lea esi,[ebx + _szGetModuleHandle]
		_invoke	[ebx + _lpGetProcAddress],eax,esi
		mov     [ebx + _lpGetModuleHandle],eax
...
REMOTE_CODE_LENGTH	equ offset REMOTE_CODE_END - offset REMOTE_CODE_START + 1
;>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
